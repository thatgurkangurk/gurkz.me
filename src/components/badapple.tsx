...
function createTable(props: { table: HTMLTableElement; height: number; width: number }) {
...
const BadApple = () => {
	...
	onMount(() => {
		...
		function fillTableCell(table: HTMLTableElement, x: number, y: number, color: string) {
			table!.rows[y]!.cells[x]!.style.backgroundColor = color;
		}

		function getRGBColorFromPixel(ctx: CanvasRenderingContext2D, x: number, y: number) {
			...
			<Switch>
				<Match when={isIOS()}>
					<Alert variant="destructive">
						...
						<AlertDescription>you are on iOS, and this will not work on there.</AlertDescription>
					</Alert>
					<Button variant="destructive" class="mt-2" onClick={() => setIsIOS(false)}>
