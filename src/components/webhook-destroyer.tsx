...
import { FormError, SubmitHandler, createForm, reset, setResponse, valiForm } from "@modular-forms/solid";
...
const WebhookDestroyer = () => {
	...
	const handleSubmit: SubmitHandler<WebhookForm> = (values) => {
		return fetch(values.url, {
			...
			.catch(() => {
				throw new FormError<WebhookForm>("something went wrong when trying to delete.");
			})
			.then((res) => {
				reset(webhookForm);
				if (res.status === 204) {
		...
		<div class="grid gap-6">
			<Form onSubmit={handleSubmit}>
				<Grid class="gap-4">
					...
					<Button type="submit" class="w-fit p-4" variant="default" disabled={webhookForm.submitting}>
						{webhookForm.submitting && <TbLoader class="mr-2 h-4 w-4 animate-spin" />}
						destroy
					</Button>
				</Grid>
			</Form>
