//! i should probably fix this later so i don't have to suppress biome so much, but for now it's fine
...
export class Cake {
	...
	blink(cursor: HTMLSpanElement) {
		const nextChar = cursor.innerHTML;
		let newChar = "_";
	...
	processLetter(type: string, lineIdx: number, letter: string) {
		...
		if (line) {
			if (letter === "newline") {
				...
				if (newLetter === ">") newLetter = "&gt;";
				if (newLetter === " ") newLetter = "&nbsp;";
				line.innerHTML = `${line.innerHTML}${newLetter}`;
			}
		}
	}
	...
	processLyricLine(idx: number) {
		...
		for (
			...
			lastLineDiv = document.getElementById(
				`lyrics${lastIndex}`,
		...
		if (lastLineDiv) {
			...
			for (
				let nextIndex = idx + 1;
				nextIndex < idx + 50 && !nextLineDiv;
				nextIndex++
			) {
				nextLineDiv = document.getElementById(
					`lyrics${nextIndex}`,
				) as HTMLSpanElement;
			}
			if (nextLineDiv) {
	...
	processCreditLine(idx: number) {
		for (
			...
			const pastLineDiv = document.getElementById(
				`credits${lastIdx}`,
			) as HTMLDivElement;
	...
	processCreditLines() {
		...
		this.creditsDelay =
			(this.creditsMaxTime * this.delayMultiplier) / totalchars;
